...
class User{
        ...
        try{
            ...
            if(!empty($this->user_name) && !empty($this->user_email) && !empty($this->user_pass)){
                ...
                if (filter_var($this->user_email, FILTER_VALIDATE_EMAIL)) { 
                    $check_email = $this->db->prepare("SELECT * FROM `tbl_user` WHERE user_email = ?");
                    ...
                    if($check_email->rowCount() > 0){
                        ...
                        $sql = "INSERT INTO `tbl_user` (username, user_email, user_password, user_image) VALUES(:username, :user_email, :user_pass, :user_image)";
    ...
    function all_users($id){
        try{
            $get_users = $this->db->prepare("SELECT id, username, user_image FROM `tbl_user` WHERE id != ?");
