...
class Friend{
    ...
    public function request_notification($my_id, $send_data){
        try{
            $sql = "SELECT sender, username, user_image FROM `friend_request` JOIN tbl_user ON friend_request.sender = tbl_user.id WHERE receiver = ?";
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->user_one == $my_id){
                            $get_user = "SELECT id, username, user_image FROM `tbl_user` WHERE id = ?";
